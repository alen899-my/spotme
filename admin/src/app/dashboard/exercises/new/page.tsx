...
const dropdownLabels = new Set(dropdownFields.map((f) => f.key))
...
export default function NewExercisePage() {
  ...
  const handleFileSelect = async (file: File | null, type: "image" | "gif") => {
    if (!file) {
      ...
      else { setGifFile(null); setGifPreview(null) }
      return
    }
    if (type === "gif") {
      setGifFile(file)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post("/admin/exercises", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      router.push("/dashboard/exercises")
    } catch {
      setErrors({ name: "Failed to create exercise. Please try again." })
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div>
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Add a new exercise to the library</p>
        </div>
      </div>
