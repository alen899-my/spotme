...
// ── Reusable dashboard data builder ──────────────────────────────────────────
async function buildDashboardData(userId) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const found = weeklyDb.find(rw => rw.day === dayStr);
    weeklyStats.push({
      date: dayStr,
  ...
  const topRecs = topExerciseRes.rows.map(row => ({
    ...row,
    scoreTag: row.rating >= 8.0 ? 'Highly Rated' : row.rating > 0 ? 'Recommended' : 'Try This',
  }));
  ...
  muscleRes.rows.forEach(row => {
    ...
    const daysAgo = (nowMs - new Date(row.workout_date).getTime()) / 86_400_000;
    const decayFactor = Math.pow(2, -daysAgo / HALF_LIFE_DAYS);
    slugScoreMap[slug] = (slugScoreMap[slug] || 0) + row.count * decayFactor;
  });

  const muscleActivity = Object.entries(slugScoreMap)
    .map(([slug, score]) => {
      const intensity = THRESHOLDS.filter(t => score >= t).length;
...
// ─── Workouts CRUD & Sessions ────────────────────────────────────────────────
async function listWorkouts({ userId, page = 1, limit = 20 }) {
  ...
  return { workouts: result.rows, total, page: pageNum, limit: limitNum };
}

async function startWorkout({ userId, title, split_id, session_id }) {
  const result = await pool.query(
    `INSERT INTO daily_workouts (user_id, title, split_id, session_id, started_at, status)
     VALUES ($1, $2, $3, $4, NOW(), 'active') RETURNING *`,
    [userId, title || null, split_id || null, session_id || null]
  );
...
async function getWorkoutById({ workoutId, userId, shared }) {
  let whereClause = 'dw.id = $1';
  const params = [parseInt(workoutId)];
  ...
  return { ...workout.rows[0], exercises: exercisesWithSets, photos: photos.rows };
}

async function addExerciseToWorkout({ workoutId, userId, exercise_id, target_sets, target_reps, target_weight, target_rest_time }) {
  const check = await pool.query(
    'SELECT id FROM daily_workouts WHERE id = $1 AND user_id = $2',
    [parseInt(workoutId), userId]
  );
  if (check.rows.length === 0) return { error: 'Unauthorized', status: 403 };
  ...
  const result = await pool.query(
    ...
    [parseInt(workoutId), exercise_id, target_sets, target_reps, target_weight, target_rest_time || '60s', sort_order]
  );
  return { exercise: result.rows[0] };
}

async function logSet({ exerciseId, set_number, weight, reps, duration_seconds, rest_seconds, workout_duration, total_rest_duration, is_skipped }) {
  const result = await pool.query(
    `INSERT INTO daily_workout_sets
       (daily_exercise_id, set_number, weight, reps, duration_seconds, rest_seconds, is_skipped)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
    [parseInt(exerciseId), set_number, weight, reps || 0, duration_seconds, rest_seconds, is_skipped || false]
  );
  ...
  if (workout_duration !== undefined || total_rest_duration !== undefined) {
    await pool.query(
      ...
      [workout_duration, total_rest_duration, parseInt(exerciseId)]
    );
  }
  ...
  if (exerciseInfo.rows.length > 0) {
    ...
    await pool.query(
      ...
      [isSkippedUpdate, isCompleted, parseInt(exerciseId)]
    );
  }
...
async function finishWorkout({ workoutId, userId, body }) {
  const {
    total_duration_seconds, total_volume, notes, completion_photo_url,
    water_intake_liters, post_workout_weight, photos, total_rest_seconds
  } = body;
  ...
  if (existingWorkoutRes.rows.length === 0) return { error: 'Workout not found or unauthorized', status: 404 };
  ...
  if (result.rows.length === 0) {
    return { error: 'Workout not found or unauthorized', status: 404 };
  }
  ...
  if (post_workout_weight && parseFloat(post_workout_weight) > 0) {
    await pool.query(
      ...
      [userId, parseFloat(post_workout_weight), `Post-workout: ${workoutId}`]
    );
  }
...
async function deleteWorkout({ workoutId, userId }) {
  ...
  return { success: true, message: 'Workout and associated photos deleted' };
}

async function updateMetrics({ workoutId, userId, water_intake_liters, post_workout_weight, total_duration_seconds, total_rest_seconds }) {
  const result = await pool.query(
    `UPDATE daily_workouts
     ...
     RETURNING *`,
    [water_intake_liters, post_workout_weight, total_duration_seconds, total_rest_seconds, parseInt(workoutId), userId]
  );
  if (result.rows.length === 0) return null;
...
async function recalculateWorkout({ workoutId, userId }) {
  const workoutCheck = await pool.query(
    'SELECT id FROM daily_workouts WHERE id = $1 AND user_id = $2',
    [workoutId, userId]
  );
  if (workoutCheck.rows.length === 0) return null;
...
async function addPhotos({ workoutId, userId, files }) {
  const workoutCheck = await pool.query(
    'SELECT id FROM daily_workouts WHERE id = $1 AND user_id = $2',
    [parseInt(workoutId), userId]
  );
  if (workoutCheck.rows.length === 0) return { error: 'Unauthorized', status: 403 };
  ...
  const urls = files.map(file => `${publicUrl}/${file.key}`);

  for (const url of urls) {
    await pool.query(
      'INSERT INTO daily_workout_photos (daily_workout_id, photo_url) VALUES ($1, $2)',
      [parseInt(workoutId), url]
    );
  }
...
async function deletePhoto({ photoId }) {
  await pool.query('DELETE FROM daily_workout_photos WHERE id = $1', [parseInt(photoId)]);
  return { success: true };
}
...
async function deleteExercise({ exerciseId, userId }) {
  ...
  if (workoutCheck.rows.length === 0) return { error: 'Exercise not found', status: 404 };
  if (workoutCheck.rows[0].user_id !== userId) return { error: 'Forbidden', status: 403 };

  await pool.query('DELETE FROM daily_workout_sets WHERE daily_exercise_id = $1', [parseInt(exerciseId)]);
  await pool.query('DELETE FROM daily_workout_exercises WHERE id = $1', [parseInt(exerciseId)]);

  return { success: true, message: 'Exercise removed from session' };
}
...
async function deleteSet({ setId, userId }) {
  ...
  if (setCheck.rows.length === 0) return { error: 'Set not found', status: 404 };
  if (setCheck.rows[0].user_id !== userId) return { error: 'Forbidden', status: 403 };

  const exerciseId = setCheck.rows[0].daily_exercise_id;

  await pool.query('DELETE FROM daily_workout_sets WHERE id = $1', [parseInt(setId)]);
...
async function updateSet({ setId, userId, weight, reps, duration_seconds, rest_seconds }) {
  const setCheck = await pool.query(
    `SELECT dw.user_id
     FROM daily_workout_sets dws
     JOIN daily_workout_exercises dwe ON dws.daily_exercise_id = dwe.id
  ...
  queryParams.push(parseInt(setId));
  const result = await pool.query(
    `UPDATE daily_workout_sets SET ${updateFields.join(', ')} WHERE id = $${paramIdx} RETURNING *`,
    queryParams
  );

  return { set: result.rows[0] };
}

async function rateExercise({ exerciseId, userId, rating }) {
  const check = await pool.query(
  ...
  const targetExerciseId = check.rows[0].exercise_id;
  ...
  await pool.query(
    `UPDATE exercises
     ...
     WHERE id = $2`,
    [parseInt(rating), targetExerciseId]
  );
...
async function rateWorkout({ workoutId, userId, rating }) {
  const result = await pool.query(
    `UPDATE daily_workouts
     SET rating = $1
     WHERE id = $2 AND user_id = $3
     RETURNING *`,
    [parseInt(rating), parseInt(workoutId), userId]
  );
...
async function getRecommendations({ userId }) {
  ...
  return scoredSessions.slice(0, 3);
}

// ── Reports & AI ─────────────────────────────────────────────────────────────
async function generateReport({ workoutId, userId, forceRetry }) {
  ...
  try {
    ...
    if (workoutRes.rows.length === 0) {
      releaseClient();
      return { error: 'Completed workout not found', status: 404 };
    }
    ...
    const countRes = await client.query(
      `SELECT COUNT(*) FROM workout_reports WHERE user_id = $1 AND created_at >= CURRENT_DATE`,
      [userId]
    );
    if (parseInt(countRes.rows[0].count) >= 10) {
      releaseClient();
      return { error: 'Daily report limit reached. You can generate up to 10 reports per day.', status: 429 };
    }
    ...
    if (existing.rows.length > 0) {
      releaseClient();
      return { report_id: existing.rows[0].id, status: existing.rows[0].status };
    }
    ...
    setImmediate(async () => {
      ...
      try {
        ...
        const exercisesRes = await bgClient.query(`
          SELECT dwe.*,
        ...
        const setsRes = await bgClient.query(`
          ...
          return {
            ...
            computed: {
              ...
              weight_range: bestW > 0
                ? `${Math.min(...weightsArr.filter(wt => wt > 0)).toFixed(1)}-${Math.max(...weightsArr).toFixed(1)}`
                : 'bodyweight',
...
4. Output valid JSON ONLY. No markdown conversational prefixes or trailing chat.`;
        ...
        await bgClient.query(
          `UPDATE workout_reports SET summary = $1, good_things = $2, areas_to_improve = $3, recommendations = $4,
                  full_content = $5, progress_pct = 100, current_phase = 'Complete', status = 'completed'
           WHERE id = $6`,
          [
            fullContent.workout_summary || '',
            fullContent.overall_assessment || '',
            '',
            Array.isArray(fullContent.recommendations) ? fullContent.recommendations.join('\n') : '',
        ...
        const pct = fullContent && Object.keys(fullContent).length > 0 ? Math.min(Object.keys(fullContent).length * 25, 95) : 5;
        try {
          await bgClient.query(
            `UPDATE workout_reports SET progress_pct = $1, current_phase = $2 WHERE id = $3`,
            [pct, 'Some sections unavailable — partial report ready', reportId]
          );
        } catch (_) {}
      } finally {
        bgClient.release();
...
async function chatWithCoach({ workoutId, userId, message, history }) {
...
async function listReports({ userId }) {
  const result = await pool.query(`
    SELECT wr.id, wr.daily_workout_id, wr.summary, wr.created_at, wr.status,
           wr.progress_pct, wr.current_phase,
...
async function listPendingWorkouts({ userId }) {
  const result = await pool.query(`
...
async function deleteReport({ reportId, userId }) {
  ...
  try {
    ...
    await client.query(
      ...
      [userId, 'workout_report', reportId]
    );
    await client.query(
...
async function getWorkoutReport({ workoutId, userId }) {
  const result = await pool.query(
...
// ── Muscle Detail & Calendar Stats ───────────────────────────────────────────
async function getMuscleDetail({ userId, slug }) {
...
async function getCalendarStats({ userId }) {
  ...
  const data = { overall, parts, restDays };
...
async function logRestDay({ userId, date, rest_type = 'fatigue' }) {
  ...
  return { updated: isUpdated, workout };
}
...
module.exports = {
  ...
  SLUG_LABELS,
  THRESHOLDS,
  syncWorkoutCompletionAnalytics,
  completeWorkoutBackground,
  buildDashboardData,
  listWorkouts,
  startWorkout,
  getWorkoutById,
