...
/**
 * Get all active physique analyses for user and daily count.
 */
async function getUserPhysiqueAnalyses(userId) {
  ...
  return { analyses: rows, todayCount, dailyLimit: DAILY_LIMIT };
}

/**
 * Run AI physique analysis on an uploaded image.
 */
async function analyzePhysiquePhoto(userId, fileKey) {
  ...
  if (usedToday >= DAILY_LIMIT) {
    const err = new Error(`Daily limit reached. You can perform ${DAILY_LIMIT} physique analyses per day.`);
    err.status = 429;
    err.limitReached = true;
    err.usedToday = usedToday;
    err.dailyLimit = DAILY_LIMIT;
    throw err;
  }
 ...
 */
async function softDeletePhysiqueAnalysis(userId, analysisId) {
...
module.exports = {
  DAILY_LIMIT,
  getUserPhysiqueAnalyses,
  analyzePhysiquePhoto,
  softDeletePhysiqueAnalysis,
};
